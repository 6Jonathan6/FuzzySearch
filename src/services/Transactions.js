...
'27-01-2018T12:34 => 2018-01-27T12:34' 
const date = stringDate.split("-")
const yearAndTime = date[date.length - 1].split("T")
const validArray = []
validArray.push(yearAndTime[0],date[1],date[0]+ "T" + yearAndTime[1])
return new Date(validArray.join('-'))
}

function getTrans(){
    const transactions = trans.map(
        function(tran){
            tran.date = validDate(tran.date)
            return tran
        }
    )
